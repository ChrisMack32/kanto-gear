...
return function(mod)
  ...
  local function loadSibling(name)
    local source = mod.read and mod:read(name)
    if not source then return nil end
    local chunk, err = load(source, "@" .. tostring(mod.path) .. "/" .. name)
    if not chunk then
      mod.log:warn("%s did not compile: %s", name, tostring(err))
      return nil
    end
    local ok, result = pcall(chunk)
    if not ok then
      mod.log:warn("%s failed to load: %s", name, tostring(result))
      return nil
    end
    return result
  end

  local desktopBridge = loadSibling("desktop_bridge.lua")
  ...
  local game
  -- Official Gen1Recomp has no mod.battle; use the desktop fallback facade.
  local BattleBridge = loadSibling("battle_bridge.lua")
  local battleApi = mod.battle
  if not battleApi and BattleBridge then
    battleApi = BattleBridge.new(function() return game end)
    mod.log:info("using desktop battle bridge")
  end
  local active = false
  local dirty = true
  ...
  local function battleState()
    ...
    for i = #states, 1, -1 do
      ...
      if state and (state.isBattleState
          or type(state.battleKind) == "function"
          or state.kind == "wild" or state.kind == "trainer"
          or state.kind == "safari" or state.kind == "link"
          or state.kind == "oldman") then
        return state
      end
    end
  end
  ...
  local function refreshBattle()
    local nextBattle = battleApi and battleApi:snapshot() or nil
    if nextBattle then
  ...
  local function submit(kind, fields)
    if not (battle and battleApi) then return end
    intentId = intentId + 1
    fields = fields or {}
    fields.id, fields.revision, fields.kind = intentId, battle.revision, kind
    local ok, err = battleApi:submit(fields)
